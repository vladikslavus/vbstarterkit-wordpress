...
/*  https://github.com/vladikslavus/grain-fiber                                */
...
/* server settings */
const config = {
    proxy: 'vbstarterkit-wordpress',
    notify: false
};

/* source paths, production paths and paths for files we need to watch */
const theme_folder = './public/wp-content/themes/vbstarterkit-wordpress';
const path = {
    build: {
        js: `${theme_folder}/js/`,
    ...
    src: {
        js: ['./src/js/main.js'], // js entry points, string when one, array when several
        style: './src/style/main.scss', // scss entry points, string when one, array when several
        fonts: './src/fonts/**/*.*',
    },
    watch: {
        js: './src/**/*.js',
        css: './src/style/**/*.scss',
        fonts: './srs/fonts/**/*.*',
        fileswatch: [`${theme_folder}/**/*+(twig|php|tpl)`] // template files monitoring, extensions    [`www/catalog/view/theme/**/*.${fileswatch}`]
    }
};
